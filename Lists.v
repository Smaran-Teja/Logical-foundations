...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with 
    | nil => O
    | h :: t => if eqb v h then S (count v t)
                else count v t
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with 
    ...
    | h :: t => match s2 with 
              | nil => false
              | a :: b =>  if (member h (cons a b))
                            then included t (remove_one h (cons a b))
                            else false
    end
    end.
...
Theorem add_inc_count : forall (p : nat) (b: bag),
count p (add p b) = S (count p b).
Proof.
  intros p b. cbn. rewrite eqb_refl. reflexivity.
Qed.
...
Proof.
  intros l1 l2. induction l1 as [| n l' IHl'].
    ...
    - cbn. rewrite IHl'. rewrite app_assoc. reflexivity. Qed.
...
rev (n :: l') = rev l' ++ [n].
Proof.
  intros l' n. cbn. reflexivity. Qed. 
...
Proof.
  intros l. induction l as [| n l' IHl'].
    - reflexivity.
    - cbn. rewrite <- IHl'. rewrite eqb_refl. reflexivity. 
  Qed.  
(** [] *)
